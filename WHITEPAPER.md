...
**Forward secrecy vs. recoverability.** The messaging *identity* is derived from the recovery phrase, so restoring the phrase on a new device recovers the account, contacts, and provable presence to peers. Ratchet *session state* is ephemeral and device-local, so the ratcheted body of a conversation is **not** recoverable from the phrase alone — those chain keys were random and never left the device.

**The session-opening message is the exception, and it is a real one.** `CryptoIdentity.derive(fromSeedPhrase:)` derives the DH identity key, the signed prekey and the PQ prekey seed deterministically from the phrase, and `prekeyBundle()` currently publishes `oneTimePrekeyPublic: nil` — no one-time prekeys are issued or consumed. The initial envelope is `[0x01][IK_A][EK_A][ciphertext]` and lives on the public transport permanently. An adversary holding the recovery phrase can therefore re-derive every recipient-side input to X3DH, combine it with the sender's ephemeral public key from the stored envelope, and recompute the root key for the first message of each conversation. Because the PQ prekey seed is derived from the same phrase, ML-KEM-768 does not mitigate this adversary.

So the accurate claim is narrower than earlier versions of this document stated: a stolen phrase opens the *first* message of every conversation, not the conversation. Closing the gap requires one-time prekeys that are genuinely random, published, consumed and deleted — or a non-deterministic signed prekey with rotation — at the cost of the reproducible-bundle property that deterministic derivation currently buys. Until that ships, this is documented at https://privamesh.org/limitations rather than left implicit. An optional encrypted device backup would additionally weaken forward secrecy for the ratcheted body.
